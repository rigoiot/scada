...
const Index: React.FC<ScadaEditorProps> = props => {
  ...
  const toolbar_config_right = [
    ...
    'separator',
    {
      ...
      action: (e: any, t: any) => {
        ...
        if (arr.length > 1) {
          ...
          arr.forEach(element => {
            element.setRect(minX, element.getRect()?.y, element.getWidth(), element.getHeight());
          });
        }
      },
    },
    {
      ...
      action: (e: any, t: any) => {
        ...
        if (arr.length > 1) {
          ...
          arr.forEach(element => {
            element.setPosition(node.getPosition()?.x, element.getPosition()?.y);
          });
        }
      },
    },
    ...
    },
    {
      ...
      action: (e: any, t: any) => {
        ...
        if (arr.length > 1) {
          ...
          arr.forEach(element => {
            element.setRect(element.getRect()?.x, minY, element.getWidth(), element.getHeight());
          });
        }
      },
    },
    {
      ...
      action: (e: any, t: any) => {
        ...
        if (arr.length > 1) {
          ...
          arr.forEach(element => {
            element.setPosition(element.getPosition()?.x, node.getPosition()?.y);
          });
        }
      },
    },
    ...
    'separator',
    {
      ...
      toolTip: '密码',
      action: (e: any, t: { editor: { g2d: { fitContent: (arg0: boolean) => void } } }) => {
  ...
  const propertySplit = new ht.widget.SplitView(propertyTitle, propertyDiv, 'v', 28);
  ...
  // 编辑组件
  const editorSplit = new ht.widget.SplitView(rulerFrame, propertySplit, 'h', -220);
          ...
          for (let i = 0; i < results?.length; i++) {
            try {
              results[i].userComponentData.indexOf('.json') === -1
                ...
                : await request.get(results[i].userComponentData).then(response => {
                    if (response instanceof Object) {
                      results[i].image = results[i].userComponentData.split('?')[0];
                    } else {
  ...
  // 处理组件数据
  const componentsData = async (data: any[]) => {
    let c: {
      groupID: string;
      items: any[];
    }[] = [];
    ...
    for (let j = 0; j < c.length; j++) {
      for (let i = 0; i < c[j].items.length; i++) {
        try {
          await request.get(c[j].items[i].componentData).then(response => {
            if (/^{/.test(response)) {
              try {
                JSON.parse(response);
                c[j].items[i].image = JSON.parse(response);
              } catch (error) {
                c[j].items[i].image = response;
              }
            } else {
              c[j].items[i].image = response;
            }
  ...
  const onSelectView = (val: { modelData: any; modelProperty: any }) => {
    ...
    request.get(modelData).then(response => {
      ...
      if (!dataModel.getDataByTag('background')) {
        ...
        globalTextCol.current = dataModel.getDataByTag('background').s('text.color');
      }
      initRightToolbar();
  ...
  const renderProperty = (data: any) => {
    ...
    } else if (['IndicatorLight', 'textIndicator', 'eg'].includes(data.a('type'))) {
      renderRightToolbar(
        <PropertyModal
    ...
    if (ht.Default.isTouchable) {
      ...
      g2d.getView().addEventListener(eventType, e => {
        ...
        if (data && data.getClassName() !== 'ht.Edge') {
          ht.Default.isDoubleClick(e) && data.a('type') !== 'block' && handleModalVisible(true);
          renderProperty(data);
        }
      });
    ...
    if (paletteNode) {
      ...
      switch (item.componentType) {
        ...
        case 'barChart':
          ...
          break;
        case 'gauge':
          item.image.renderHTML = new Function('return ' + item.image.renderHTML)();
          node.setImage(item.image);
          break;
        case 'container':
  ...
  const handleFile = async (values: any) => {
    ...
    const file = new File(
      ...
      `${values.name}_${viewDataRef.current.id || viewDataRef.current.key}.json`,
      {
  ...
  const saveModel = async () => {
    ...
    return Promise.all([modelData, modelProperty]).then(values => {
      const temArr = values.map(
        rs => rs.url.substr(0, rs.url.indexOf('?')) + `?v=${new Date().getTime()}`
      );
      return updateView({
  ...
  const importData = (val: string) => {
    request.get(val).then(response => {
      handleFile({
        ...
        const url = rs?.url?.substr(0, rs?.url?.indexOf('?')) + `?v=${new Date().getTime()}`;
        updateView({
          id: viewDataRef.current.id || viewDataRef.current.key,
          modelData: url,
          tag: viewDataRef.current.tag || new Date().getTime(),
  ...
  const generalModalSubmit = (value: any, node?: any) => {
    if (value.presetSize) {
      ...
      if (isGrid.current && (presetWidth !== width || presetHeight !== height)) {
        initGrid(bagNode);
      }
      if (value.bagImg) {
      ...
      switch (node.a('type')) {
        ...
        case 'time':
          ...
          let temDateFormat = value.dateFormat.replace(/-/g, value.dateSeparator);
          ...
          const date = value.dateCheck ? moment(new Date()).format(temDateFormat) : '';
          const week = value.weekCheck ? '周' + '日一二三四五六'.charAt(new Date().getDay()) : '';
          const time = value.timeCheck ? moment(new Date()).format(value.timeFormat) : '';
          node.a('date', `${date} ${week} ${time}`);
          break;
        case 'switch':
  ...
  const propertyModalSubmit = (value: any, node: any) => {
    ...
    value.type === 'IndicatorLight' && node.a('indicatorLight', value.indicatorLight);
    ['textIndicator', 'eg'].includes(value.type) && node.a('textData', value.textData);
    publicDataValue(value, node);
    if (value.type === 'textIndicator') {
      switch (value.indicator) {
        case 'text':
          node.a('indicator') !== 'text' && node.setImage(require('./editor/textIndicator.json'));
          node.s('text.align', value.textAlign);
          node.s('text.vAlign', value.textVAlign);
          node.s('text.color', value.textColor || undefined);
          node.s('text.font', value.textFont);
  ...
  return (
    ...
    >
      ...
      {modalVisible && ['IndicatorLight', 'textIndicator', 'eg'].includes(values.type) && (
        <PropertyModal
          onSubmit={(value: any) => {
      ...
      {modalVisible && ['chart', 'gauge', 'barChart', 'container', 'pie'].includes(values.type) && (
        <ChartModal
          ...
          onSubmit={val => {
            ...
            if (typeof Image === 'object' && val.selectData) {
              val.selectData?.item?.userComponentData
                ? data.setImage(val.selectData?.item?.userComponentData?.split('?')[0])
                : data.setImage(val.selectData?.item?.componentData?.split('?')[0]);
